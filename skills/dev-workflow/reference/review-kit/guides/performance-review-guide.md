...
| 指标 | 全称 | 目标值 | 含义 |
|------|------|--------|------|
| **LCP** | 最大内容绘制时间（Largest Contentful Paint） | ≤ 2.5s | 最大内容绘制时间 |
| **INP** | 交互到下一次绘制（Interaction to Next Paint） | ≤ 200ms | 交互响应时间（2024 年替代 FID）|
| **CLS** | 累积布局偏移（Cumulative Layout Shift） | ≤ 0.1 | 累积布局偏移 |
| **FCP** | 首次内容绘制（First Contentful Paint） | ≤ 1.8s | 首次内容绘制 |
| **TBT** | 总阻塞时间（Total Blocking Time） | ≤ 200ms | 主线程阻塞时间 |
...
| 工具 | 用途 |
|------|------|
| Chrome 开发者工具（Memory） | 堆快照分析 |
| MemLab（Meta） | 自动化内存泄漏检测 |
| 性能监视器（Performance Monitor） | 实时内存监控 |
...
| FCP | ≤ 1.8s | 1.8-3s | > 3s |
| JS 包体积（Bundle Size） | < 200KB | 200-500KB | > 500KB |
...
| 工具 | 用途 |
|------|------|
| [Lighthouse](https://developer.chrome.com/docs/lighthouse/) | 核心 Web 指标测试 |
| [WebPageTest](https://www.webpagetest.org/) | 详细性能分析 |
| [webpack-bundle-analyzer](https://github.com/webpack-contrib/webpack-bundle-analyzer) | 包体积分析 |
